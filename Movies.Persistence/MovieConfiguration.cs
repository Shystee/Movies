...
public class MovieConfiguration : IEntityTypeConfiguration<Movie>
{
	public void Configure(EntityTypeBuilder<Movie> builder)
	{
		...
		ConfigureMovieDirectorTable(builder);
		ConfigureMovieWriterTable(builder);
	...
	private static void ConfigureIsActive(EntityTypeBuilder<Movie> builder)
	{
	...
	private static void ConfigureMovieCastTable(EntityTypeBuilder<Movie> builder)
	{
		...
		builder.Navigation(m => m.Cast).UsePropertyAccessMode(PropertyAccessMode.Field);
	}

	private static void ConfigureMovieDirectorTable(EntityTypeBuilder<Movie> builder)
	{
		builder
			.HasMany(m => m.Directors)
			.WithMany()
			.UsingEntity<Dictionary<string, object>>(
				TableNames.MovieDirectors,
				j => j
					.HasOne<Person>()
					.WithMany()
					.HasForeignKey("person_id")
		...
		builder.Navigation(m => m.Directors).UsePropertyAccessMode(PropertyAccessMode.Field);
	}
	...
	private static void ConfigureMovieWriterTable(EntityTypeBuilder<Movie> builder)
	{
		builder
			.HasMany(m => m.Writers)
			.WithMany()
			.UsingEntity<Dictionary<string, object>>(
				TableNames.MovieWriters,
				j => j
					.HasOne<Person>()
					.WithMany()
					.HasForeignKey("person_id")
		...
		builder.Navigation(m => m.Writers).UsePropertyAccessMode(PropertyAccessMode.Field);
	}
	...
	private static void ConfigureUrls(EntityTypeBuilder<Movie> builder)
	{
		builder.OwnsOne(m => m.PosterUrl, url =>
		{
		...
		builder.OwnsOne(m => m.BackdropUrl, url =>
		{
			url.Property(u => u.Value)
		...
		builder.OwnsOne(m => m.TrailerUrl, url =>
		{
			url.Property(u => u.Value)
